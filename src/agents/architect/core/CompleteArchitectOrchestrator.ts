...
Discord: ${buildResult.discordConfigured ? ‘Configured’ : ‘Needs setup’}

Your new intelligent agent is ready! 🚀`, { type: 'creation' }); } else { return await this.voice.formatResponse( `❌ Agent creation failed: ${buildResult.error || ‘Unknown error’}`, { type: 'error' } ); } } catch (error) { return await this.voice.formatResponse( `Agent creation failed: ${error instanceof Error ? error.message : ‘Unknown error’}`,
{ type: ‘error’ }
);
}
}
...
Suggestions: ${analysis.suggestions.join(’, ’)}`, { type: 'analysis' }); } catch (error) { return await this.voice.formatResponse( `Code analysis failed: ${error instanceof Error ? error.message : ‘Unknown error’}`,
{ type: ‘error’ }
);
}
}
